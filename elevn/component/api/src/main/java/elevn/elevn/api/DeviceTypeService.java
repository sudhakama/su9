...
package elevn.elevn.api;

import elevn.elevn.api.dto.DeviceJSON;
...
@API(name = "elevn", version = "1.0.0", context = "/elevn", tags = "elevn")
@DeviceType(value = "elevn")
public interface DeviceTypeService {
    ...
    @Permission(scope = "elevn_user", permissions = {"/permission/admin/device-mgt/user/register"})
    ...
    @Permission(scope = "elevn_user", permissions = {"/permission/admin/device-mgt/change-status"})
    Response changeStatus(@PathParam("deviceId") String deviceId,
                          @QueryParam("state") String state,
                          @Context HttpServletResponse response);

    /**
     * Retrieve Sensor data for the elevn
     */
    @Path("device/stats/{deviceId}")
    @GET
    @Consumes("application/json")
    @Produces("application/json")
    @Permission(scope = "elevn_user", permissions = {"/permission/admin/device-mgt/stats"})
    Response getSensorStats(@PathParam("deviceId") String deviceId, @QueryParam("from") long from,
                            @QueryParam("to") long to);

    @Path("/device/{device_id}")
    @DELETE
    @Permission(scope = "elevn_user", permissions = {"/permission/admin/device-mgt/removeDevice"})
    Response removeDevice(@PathParam("device_id") String deviceId);

    @Path("/device/{device_id}")
    @PUT
    @Permission(scope = "elevn_user", permissions = {"/permission/admin/device-mgt/updateDevice"})
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @Permission(scope = "elevn_user", permissions = {"/permission/admin/device-mgt/devices"})
    Response getAllDevices();

    @Path("/device/download")
    @GET
    @Produces("application/zip")
    @Permission(scope = "elevn_user", permissions = {"/permission/admin/device-mgt/download"})
